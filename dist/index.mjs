...
// node_modules/esprima-next/dist/esm/esprima.js
var Syntax;
(function(Syntax2) {
...
var worker_default = {
  fetch: async (req, env) => {
    ...
    if (rootPath)
      return new Response(JSON.stringify({ api, examples, user }, null, 2), { headers: { "content-type": "application/json; charset=utf-8" } });
    console.log(pathOptions);
    const allMethods = Object.keys(import_lodash.default);
    let methods = [];
    let segments = pathSegments;
    while (import_lodash.default[segments[0]]) {
      let len = 1;
      const name = segments[0];
      let args = segments[1].includes(":") ? [segments[1].split(",").reduce((acc, keyValue) => ({ ...acc, [keyValue.split(":")[0]]: keyValue.split(":")[1] }), {})] : segments[1].split(",");
      console.log(
        segments[0],
        import_lodash.default[args[0]]
      );
      if (!import_lodash.default[args[0]]) {
        len = 2;
      } else {
        args = [];
      }
    ...
    let target = segments;
    let data, output, dataInProcess, error = void 0;
    console.log(target);
    const source = target.length > 0 ? "https://" + target.join("/") + search : void 0;
    let steps = [];
    let chain = void 0;
    try {
      ...
      for (let method2 of methods) {
        output = method2.args.length == 1 ? import_lodash.default[method2.name](data, method2.args[0]) : import_lodash.default[method2.name](data, [...method2.args]);
        steps.push({ method: method2, data: output });
        data = output;
      }
      if (output)
    ...
    if (error || pathOptions?.debug)
      return new Response(JSON.stringify({ api, methods, steps, source, data, output, error, user }, null, 2), { headers: { "content-type": "application/json; charset=utf-8" } });
